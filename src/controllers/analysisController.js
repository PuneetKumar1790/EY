...
async function analyzeTenderEligibility(req, res) {
  try {
    ...
    console.log(`✓ Concatenated comprehensive summary (${comprehensiveSummary.length} characters)`);
    ...
    if (!await fs.pathExists(companyInfoPath)) {
      return res.status(400).json({
        success: false,
        error: 'Company information not found. Please upload company document first.'
      });
    }
